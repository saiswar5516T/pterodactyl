...
<head>
    ...
    <body style="background: var(--theme-bg);">
        ...
        <div class="container">
            <div class="sidebar">
                ...
                <!-- Website Name -->
                <a class="logo-expand" href="/">{{ config('app.name', 'Pterodactyl') }}</a>
    ...
    <path d="M12,5V4c0-2.209-1.791-4-4-4S4,1.791,4,4v1c0,2.209,1.791,4,4,4S12,7.209,12,5z" style="
        fill: #FFFF;"/>
                        ...
                        <a class="sidebar-link Logout" href="/auth/logout">
                            <svg viewBox="0 0 24 24" fill="currentColor">
            ...
            <div class="wrapper">
                <div class="header">
                    <div class="search-bar" onclick="Search()">
                        <input type="text" placeholder="{{ config('unixlang.search', 'Search') }}">
                    </div>
                    <div class="user-settings">
                        ...
                        <a href="/account"><i class="fas fa-user fa-xm"></i> {{ config('unixlang.account', 'Account') }}</a>
                        @if(Auth::user()->root_admin) <a href="/admin"><i class="fas fa-user-cog fa-xm"></i> {{ config('unixlang.admin', 'Admin') }}</a>@endif
                        <a href="/auth/logout"><i class="fas fa-sign-out-alt fa-xm"></i> {{ config('unixlang.logout', 'Logout') }}</a>
                        </div>
                    </div>
                    </div>
                </div>
    ...
    <style>
      ...
      --button-bg: @isset($setting_data['sb-button-bg']){{$setting_data['sb-button-bg']}}@else #353340  @endisset;;
      ...
      --delay: 0s;
          ...
          .hTuBzu {
            ...
            .dMZZHB {
                ...
                background-color: @isset($setting_data['buttoncolor']){{$setting_data['buttoncolor']}}@else #0045ff @endisset;;
            }
            p, .bVAiVP, .hSKhGC, .ka-difk, .jdiuFH  , .fNmetC , .hcJQtJ  , .lOfYt , .kAWZHe  , .gxlLzB, .kmjiH, .epSLdr, .cWFcHc > div > a:not(:first-of-type), .cWFcHc > div > div:not(:first-of-type)  {
      color: var(--body-color) !important;
    }
    
    .ebtnLL #logo > a, .sidebar .logo-expand, .sidebar-link:hover, .sidebar-link.is-active, .eBRJeB,.cKOMAz , .kmjiH, .dHRxAp{
        color: var(--body-active-color) !important;
    }
    
    .cWFcHc > div > a:active, .cWFcHc > div > div:active, .cWFcHc > div > a.active, .cWFcHc > div > div.active {
        color: var(--body-active-color) !important;
    }
